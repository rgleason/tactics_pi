...
void TacticsInstrument_Clock::SetData( int, double, wxString )
{
// Nothing to do here but we want to override the default
}

void TacticsInstrument_Clock::SetUtcTime( wxDateTime data )
{
    if (data.IsValid())
    {
...
void TacticsInstrument_Moon::SetUtcTime( wxDateTime data )
{
    if (data.IsValid())
    {
...
int TacticsInstrument_Moon::moon_phase(int y, int m, int d)
{
    /*
...
TacticsInstrument_Sun::TacticsInstrument_Sun( wxWindow *parent, wxWindowID id, wxString title ) :
    TacticsInstrument_Clock( parent, id, title, OCPN_DBP_STC_LAT|OCPN_DBP_STC_LON|OCPN_DBP_STC_CLK )
{
    m_lat = m_lon = 999.9;
...
void TacticsInstrument_Sun::SetData( int st, double data, wxString unit )
{
      if( st == OCPN_DBP_STC_LAT )
      {
            m_lat = data;
